...
#   ) |> 
#   ungroup()
# write.table(table, "clipboard-128", sep = "\t", row.names = F)
# 
# # Test Emissions
# table <- energy_total |> 
#   filter(
...
#   ) |> 
#   summarize(
#     CO2 = sum(`Emissions (CO2)`, na.rm = T),
#     CH4 = sum(`Emissions (CO2eq) from CH4 (AR5)`, na.rm = T),
#     N2O = sum(`Emissions (CO2eq) from N2O (AR5)`, na.rm = T)
#   ) |> 
#   ungroup()
# write.table(table, paste0("clipboard-",2^20), sep = "\t", row.names = F)
```
...
energy_addition <- energy_total |> 
  mutate(
    Sector = if_else(Sector == "Consumption by electricity, chp and heat plants", "Electricity, chp and heat plants", Sector),
    `ISIC Code` = if_else(
