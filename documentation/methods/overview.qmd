...
As our goal is to leverage valuable emissions data from different sources, we have scripted data processing routines to ingest original datasets in tabular format (mainly Excel Files and Comma Separated Values) and manipulate them to conform to a single file structure, using the R programming language [@Rcore2024][^1]. The flow of data follows the diagram in @fig-dataflow.
