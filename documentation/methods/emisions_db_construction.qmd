...
library(tidyverse)
library(gt)
gwp_faostat_total <- readRDS(
  "gears_db/data/outputs/gwp_faostat_total.rds"
)
un_energy <- readRDS(
...
## Emissions from industrial processes and waste
...
Data on CO~2~, CH~4~, and N~2~O emissions from Agriculture, Forestry, and Other Land Use, including Land Use Change were sourced from FAOSTAT[^2]. The level of disaggregation of its individual datasets allows us to identify the individual agricultural product or asset (in the case of animals) behind the emission of a specific gas. Since values are expressed in kilotonnes (kt) of each GHG, we converted it to Global Warming Potential AR5 for a 100-year horizon using the values in @tbl-conversion-ar5. 

To facilitate the mapping to CGE and Input-Output frameworks, we added a correspondence to individual ISIC economic activities responsible for the emission, where possible.  
...
```{r}
#| echo: false
#| tbl-label: tbl-faostat-isic
#| tbl-cap: ISIC sectors used with FAOSTAT elements

data.frame(Code = unique(gwp_total_emissions$`ISIC Code`[startsWith(gwp_total_emissions$`IPCC Code`, "3")]), Sector = unique(gwp_total_emissions$ISIC[startsWith(gwp_total_emissions$`IPCC Code`, "3")])) |> 
  gt()
```

## Greenhouse Gas Emissions from Fuel Combustion and Fugitive Emissions

A choice was made to estimate emissions from combustion, although the global datasets that we use already have this information in the aggregate, because it was important to be able to distinguish between the different fuels driving the combustion and the sectors in which this happened. Greenhouse gas emissions from fuel combustion and fugitive emissions were estimated applying Tier 1 coefficients from the 2006 IPCC Guidelines for National Greenhouse Gas Inventories to national information on fuel uses from the United Nations Energy Database [@unsd2023] using @eq-emission-factors.
