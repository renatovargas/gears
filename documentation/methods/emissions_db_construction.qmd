...
We use the case of Nigeria to exemplify differences between datasets in @tbl-nigeria-dataset-comparison. The first column shows the result of our MAFAP Emissions Dabase, the second shows the original Edgar dataset values, the third one shows a modified version of EDGAR, in which the whole section of agriculture has been replaced with FAOSTAT data (although for this comparison we removed the LULUCF part from FAO), and the last column shows the values of the World Bank World Development Indicators (only for Greenhouse Gases CO~2~, CH~4~, and N~2~O in CO~2~eq). The rows show the different emission types. Variations between datasets range from less than 1% to more than 30%. While variations remain stable with CO~2~ and CH~4~, CH~4~ is more volatile and there is a complete discrepancy between our dataset and the Edgar based calculations of CO~2~bio, which at this  point we only can attribute to differences in elements considered for this sector, as EDGAR documentation regarding this matter appears vague in the main publication [@crippa2023]. In our case, the CO~2~ emissions of the combustion of bio jet kerosene, biodiesel, biogasoline, charcoal, biogases, bagasse, fuelwood, other liquid biofuels, animal waste, municipal waste, and vegetal waste were considered CO~2~bio, while their CH~4~ and N~2~O emissions were classified together with the emission of those gases of all other fuels.



| Emission Type                     | MAFAP EMDB | EDGAR     | EDGAR + FAOSTAT | World Bank |
| :---- | --: | --: | --: | --: |
...
An important value added to the underlying data sources of MAFAP Emissions Database is an initial classification of emissions according to ISIC sector. This is important, because it allows practitioners of Computable General Equilibrium analysis to bridge this data with Social Accounting Matrices (which are the databases for such analyses). We made the choice to disregard the information on energy intermediate and final consumption of industries outside the energy production and transformation sectors, since as per the conventions of energy balances, fuels consumed by any sector get aggregated into the transportation sector, but that does not coincide with the corresponding ISIC classification for road transportation. Because of that, we pooled together all consumption and classified it as "Undetermined" for practitioners to adjudicate it according to either input shares, endowment shares, or output shares. @tbl-total-emissions-by-isic shows this exercise for Nigeria in 2022.
...
As part of the Monitoring and Analyzing Food and Agricultural Policies (MAFAP) program, we sourced data from EDGAR [@crippa2023], FAOSTAT [@fao2024], and the United Nations Energy Database [@unsd2023] to provide a bridge between emissions information and social accounting matrices for Computable General Equilibrium analysis. 
